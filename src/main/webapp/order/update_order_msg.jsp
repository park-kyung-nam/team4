    ...
    <UL>
      <c:choose>
        <c:when test="${param.count == 1 }">
          <LI class='li_none'>주문 정보를 변경했습니다.</LI>
        </c:when>
        <c:otherwise>
          <LI class='li_none'>주문 정보 변경에 실패했습니다.</LI>
          <LI class='li_none'>다시한번 시도해주세요.</LI>
        </c:otherwise>
      </c:choose>
      <LI class='li_none'>
        <br>     <!-- http://localhost:9090/team4/order/list_order.do?customerno=1 -->
        <button type='button' onclick="location.href='./list_order.do?customerno=${param.customerno}'">목록</button>
      </LI> 
    </UL>
  </fieldset>
