...
<DIV class='title_line'>주문 삭제 알림</DIV>
    ...
    <UL>
      <c:choose>
        <c:when test="${param.count == 1 }">
          <LI class='li_none'>${param.orderno } 주문을 삭제했습니다.</LI>
        </c:when>
        <c:otherwise>
          <LI class='li_none'>${param.orderno } 주문 삭제에 실패했습니다.</LI>
        </c:otherwise>
      </c:choose>
      <LI class='li_none'>
        <br>
        ...
        <button type='button' onclick="location.href='./list_order.do?customerno=1'"  class="btn btn-primary btn-md">목록</button>
      </LI>
