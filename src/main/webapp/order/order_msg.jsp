...
<DIV class='title_order_msg'>알림</DIV>
    ...
    <UL>
      <c:choose>
        <c:when test="${param.count == 1 }">
          <LI class='li_none'>주문이 정상적으로 처리되었습니다.</LI>
          
        </c:when>
        <c:otherwise>
          <LI class='li_none'>주문 처리에 실패하였습니다.</LI> 
        </c:otherwise>
      </c:choose>
      <LI class='li_none'> 
        <br>          <!-- http://localhost:9090/team4/order/list_order.do?customerno=11 -->
        <button type='button' onclick="location.href='./list_order.do?customerno=${param.customerno}&productno=${param.productno }'">주문 확인</button>
        <button type='button' onclick="location.href='../index.jsp'">메인 화면으로</button>
      </LI>
    </UL> 
