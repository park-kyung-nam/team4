...
<ASIDE style='float: left;'>
 <A href='./list_by_search_paging.do?nowPage=${param.nowPage}' >중고 매입</A> 
 ...
 <TABLE class='table table-striped'>
  ...
  <%-- table 내용 --%>
    ...
    <c:forEach var="usedVO" items="${list }" varStatus="info">
  <TR>
    <TD style='text-align: center;'>${info.count } </TD>   
  
    <td style='vertical-align: middle;'>
              <c:choose>
                <c:when test="${usedVO.ansnum == 0 }"> <!-- 부모글인 경우 -->
                  <img src='./images/ting1.png'>
                </c:when>
                <c:when test="${usedVO.ansnum > 0}">    <!-- 답변글인 경우 -->
                  <img src='./images/white.png' style='width: ${usedVO.indent * 20}px; opacity: 0.0;'>
                  <img src='./images/reply.png'>
                </c:when>
              </c:choose> 
    <a href="./read.do?usedno=${usedVO.usedno}&word=${param.word}&nowPage=${param.nowPage}">${usedVO.u_title }</a>
    </td>
    <TD style='text-align: center;'>${usedVO.rname } </TD>   
    <td style='text-align: center ;'>${usedVO.rdate.substring(0, 10) }</td>
    <TD>
      <A href="./update.do?usedno=${usedVO.usedno }&word=${param.word}&nowPage=${param.nowPage}"><IMG src='./images/update.png' title='수정'></A>
      <A href='./delete.do?usedno=${usedVO.usedno }&word=${param.word}&nowPage=${param.nowPage}'><IMG src='./images/delete.png' title='삭제'></A>
      <A href='./reply.do?usedno=${usedVO.usedno }&word=${param.word}&nowPage=${param.nowPage}'><IMG src='./images/reply.png' title='답변'></A>
      [G${usedVO.grpno}/A${usedVO.ansnum}]
    </TD>
  </TR>
  </c:forEach> 
  </tbody>
