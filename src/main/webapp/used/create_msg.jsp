    ...
    <UL>
      <c:choose>
        <c:when test="${param.count == 1 }">
          <LI class='li_none'>리뷰를 등록했습니다.</LI>
        </c:when>
        <c:otherwise>
          <LI class='li_none'>리뷰 등록에 실패했습니다.</LI>
        </c:otherwise>
      </c:choose>
      <LI class='li_none'>
        <br>
        <button type='button' onclick="location.href='./create.do'">새로운 리뷰 등록</button>
        <button type='button' onclick="location.href='./list_by_search_paging.do?nowPage=${param.nowPage}'">목록</button>
      </LI>
     </UL>
  </fieldset>
